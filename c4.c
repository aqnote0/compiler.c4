...
int *e, *le, // current position in emitted code
    *id,     // currently parsed identifier (标识)
    *sym,    // symbol table (simple list of identifiers) (符号表)
    tk,      // current token (令牌)
    ival,    // current token value (令牌值)
    ty,      // current expression type (表达式类型)
...
enum tokens {
  Num = 128, Fun, Sys, 
  Glo, Loc, Id, Char, Else, 
  Enum, If, Int, Return, Sizeof, 
...
const char *tokenNames[] = {"Num", "Fun", "Sys", "Glo", "Loc", "Id", "Char", "Else", "Enum", 
  "If", "Int", "Return", "Sizeof", "While", "Assign", "Cond", "Lor", "Lan", "Or", "Xor", 
  "And", "Eq", "Ne", "Lt", "Gt", "Le", "Ge", "Shl", "Shr", "Add", "Sub", "Mul", "Div", "Mod", 
  "Inc", "Dec", "Brak"};
const char *opcodeNames[] = {"LEA", "IMM", "JMP", "JSR", "BZ", "BNZ", "ENT", "ADJ", "LEV", "LI", 
  "LC", "SI", "SC", "PSH", "OR", "XOR", "AND", "EQ", "NE", "LT", "GT", "LE", "GE", "SHL", 
  "SHR", "ADD", "SUB", "MUL", "DIV", "MOD", "OPEN", "READ", "CLOS", "PRTF", "MALC", 
  "FREE", "MSET", "MCMP", "EXIT"};

const char *typeNames[] = {"CHAR", "INT", "PTR"};
...
int main(int argc, char **argv) {
    ...
    if ((i = read(fd, p, poolsz - 1)) <= 0) { // 读取文件到p内存中
    ...
    while (tk) {
        ...
        if (tk == Int)
            ...
            if (tk == '{') {
                ...
                i = 0;
                while (tk != '}') {
                    if (tk != Id) { // 枚举括号中包含为非变量token错误
                        printf("%d: bad enum identifier %d\n", line, tk);
                        return -1;
                    }
                    next();
                    if (tk == Assign) { // 枚举变量赋值逻辑处理
                        next();
                        if (tk != Num) {
        ...
        while (tk != ';' && tk != '}') {
            ...
            if (tk != Id) { // 类型后面没跟“标识符”错误，“标识符”可能是“符号名”、“变量名”
            ...
            if (tk == '(') { // 函数处理流程
                ...
                while (tk != ')') { // 函数参数处理流程
                    ...
                    if (tk == Int) // int形参数处理
                        next();
                    else if (tk == Char) { // char形参数处理
                        next();
                        ty = CHAR;
                    }
                    while (tk == Mul) { // 指正形参数处理
                        next();
                        ty = ty + PTR;
                    }
                    if (tk != Id) { // 变量异常处理流程
                ...
                loc = ++i;
                next();
                while (tk == Int || tk == Char) { // 函数体处理流程
                    bt = (tk == Int) ? INT : CHAR;
                    next();
                    while (tk != ';') { // 定义处理流程（该流程无法处理 “定义+赋值”）
                        ty = bt;
                        while (tk == Mul) { // 指针类型处理
                            next();
                            ty = ty + PTR;
                        ...
                        if (id[Class] == Loc) { // 类型后面重复变量错误
                            printf("%d: duplicate local definition\n", line);
                            return -1;
                        }
                        id[HClass] = id[Class]; // 记录token
                        id[Class] = Loc;
                        id[HType] = id[Type];
                        id[Type] = ty;
                        id[HVal] = id[Val];
